...
import { PixiSpriteWinPopup } from "./pixiSpriteWinPopup.js";
...
export class GameScene {
	...
	async init() {
		...
		if (this.winPopupOptions.useSprite) {
			this.winPopup = new PixiSpriteWinPopup({
				app: this.app,
				config: {
					scale: 1,
					offsetX: 90,
			...
			await this.winPopup.init(
				"/ScratchCard_Gems-Web/assets/sprites/winPopup (Bling).png"
			);
		} else {
